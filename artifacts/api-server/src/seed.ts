...
export async function seedDemoData() {
  // Fast guard: skip entirely when demo data is already present.
  // This makes cold-starts cheap on serverless (one COUNT query vs 700+ INSERTs).
  try {
    const check = await db.execute(sql`SELECT COUNT(*)::int AS c FROM users`);
    const c = Number((check.rows[0] as Record<string, unknown>).c ?? 0);
    if (c > 0) {
      logger.info({ c }, "Demo data already seeded – skipping");
      return;
    }
  } catch {
    // Table may not exist yet on a fresh database — fall through and seed.
  }
